...
class ReluTransReadWrite(object):
    def __init__(self, training_batch_size=25, source_vocab_size=50001, target_vocab_size=50001,
    ...
    def symbolic_elbo(self, source, target, source_l, target_l):
        ...
        e = get_output(self.out_mlp, h_init)
        attention_init = get_output(self.attention, e[:, :300])
    ...
    def step(self, h1, canvas, start, stop, ref, s_idx, t_idx):
        ...
        read_stop_pos = read_start_pos + (1.0 - read_start_pos) * stop[:, 0]
        ...
        write_stop_pos = write_start_pos + (1.0 - write_start_pos) * stop[:, 1]
    ...
    def decode_fn(self):
        ...
        # Forward path
        ([prob, embed, prev_idx, candidate_idx, tops], updates) = theano.scan(self.forward_step, sequences=[canvas[1:]],
                                                                              outputs_info=[prob0, prev_embed_init, None, None, None],
                                                                              non_sequences=[candidate_embed])
        last_prob = prob[-1]
        last_idx = T.cast(T.argmax(last_prob, axis=-1), "int32")
        # Backward

        ([i, decodes], update) = theano.scan(self.backward_step, sequences=[prev_idx, candidate_idx],
                                             outputs_info=[last_idx, None],
                                             go_backwards=True)

        decode_fn = theano.function(inputs=[source, source_l], outputs=[prev_idx, candidate_idx, tops, decodes])
        return decode_fn
    ...
    def forward_step(self, canvas_info, prob, prev_embed, candate_embed):
        ...
        prev_idx = T.cast(T.floor(T.true_div(top_k, self.target_vocab_size)), "int32")
...
def decode_process():
    ...
    with open("SentenceData/WMT/Data/data_idx.txt", "r") as dataset:
        ...
        l = data[:, 2]
        l = np.array(l.tolist()).astype("int8")
        en_l = l[:, 0]
        output = decode_process(en_batch, en_l)