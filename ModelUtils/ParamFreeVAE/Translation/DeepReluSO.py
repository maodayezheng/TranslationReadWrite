...
class DeepReluTransReadWrite(object):
    ...
    def step(self, t_s, h1, h2, a_p, canvas, r_a, w_a, ref, r_p, w_p):
        ...
        offset = attention[:, :2]
        scale = attention[:, 2:]
        read_attention = T.nnet.relu(1.0 - T.abs_(2.0*(r_p - offset[:, 0].reshape((n, 1)))/(scale[:, 0].reshape((n, 1))+ 1e-5) - 1.0))
        write_attention = T.nnet.relu(1.0 - T.abs_(2.0*(w_p - offset[:, 1].reshape((n, 1)))/(scale[:, 1].reshape((n, 1))+ 1e-5) - 1.0))
    ...
    def elbo_fn(self):
        ...
        source = T.imatrix('source')
        target = T.imatrix('target')
        reconstruction_loss, read_attention, write_attetion = self.symbolic_elbo(source, target, None)
