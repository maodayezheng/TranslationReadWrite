...
class DeepReluTransReadWrite(object):
    ...
    def symbolic_elbo(self, source, target):
        ...
        t_l = time_steps.shape[0]
        ([h_t_1, h_t_2, a_t, encode_info, read_attention], update) \
            = theano.scan(self.step, outputs_info=[h_init, h_init, read_attention_init, None, None],
                          non_sequences=[read_pos, source_embedding, read_attention_weight, read_attention_bias],
                          sequences=[time_steps.reshape((t_l, n, 1))])
        ...
        ([h_t_1, h_t_2, s, sample_score], update) = theano.scan(self.decoding_step,
                                                                ...
                                                                sequences=[decode_in_embedding, encode_info])
    ...
    def step(self, t_s, h1, h2, a, r_p, ref, r_a_w, r_a_b):
        n = h1.shape[0]
        # Compute the read and write attention
        address = T.nnet.sigmoid(T.dot(a, r_a_w) + r_a_b)
        offset = address[:, 0]
        scale = address[:, 1]
    ...
    def decoding_step(self, embedding, encode_info, h1, h2, s_embedding):
    ...
    def get_params(self):
        ...
        return input_embedding_param + target_input_embedding_param + target_output_embedding_param + \
               ...
               out_param + score_param + decode_out_param + [self.attention_weight, self.attention_bias]
...
def run(out_dir):
    print("Run the Relu Interval Offset Read ")
