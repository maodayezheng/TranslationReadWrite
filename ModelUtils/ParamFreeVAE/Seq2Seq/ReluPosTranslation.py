...
def build_model(source_vocab_size=1026, target_vocab_size=1247,
                embed_dim=600, hid_dim=3072, source_seq_len=50,
    ...
    v = np.random.uniform(low=-1.0, high=1.0, size=(hid_dim, 4+target_seq_len+source_seq_len))
...
def run(main_dir=None, out_dir=None, load_param_dir=None, pre_trained=False):
    ...
    val_elbo = 1000
    target_vocab = []
    with open("SentenceData/WMT/100data-test/vocab_de", "r") as de:
        for line in de:
