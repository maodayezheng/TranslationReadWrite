...
Simple read and write:
 1. The source sentence is encoded by the BOW
 2. Only write position is predict by model
...
def build_model(source_vocab_size=1026, target_vocab_size=1247,
    ...
    v = np.random.uniform(low=-0.001, high=0.001, size=(hid_dim, 2+target_seq_len))
    w_loc = theano.shared(name="w_source_loc", value=v.astype(theano.config.floatX))
    params.append(w_loc)

    v = np.zeros((2+target_seq_len,))
    ...
    def step(h, canvas, s, t_index, mask,
        ...
        t_start = pos_params[:, 0]
        t_stop = t_start + (1 - t_start) * pos_params[:, 1]
        t_strength = pos_params[:, 2:]
        ...
        return h1, canvas, target_p

    # RNN
    source_read = T.sum(source_sentence * w_source_pos.reshape((1, source_seq_len, embed_dim)), axis=1)
    w_write = w_target_write_pos.reshape((1, target_seq_len, embed_dim))
    ([h1, canvases, target_pos], u) \
        = theano.scan(step, outputs_info=[h_init, canvas_init, None],
                      non_sequences=[source_read, target_index, mask,
    ...
    optimiser = theano.function(inputs=[source, target, target_l],
                                outputs=[loss, target_pos],
                                updates=updates,
                                allow_input_downcast=True
                                )

    validater = theano.function(inputs=[source, target, target_l],
                                outputs=[loss],
                                allow_input_downcast=True
                                )
...
def run(main_dir=None, out_dir=None, load_param_dir=None, pre_trained=False):
    ...
    train_data = None
    validation = None
    print("Run the simple ReluPos RW")
    ...
    for i in range(100000):
        ...
        l = np.array(l.tolist())
        de_l = l[:, 1]
        loss, target_pos = optimiser(en_batch, de_batch, de_l)
